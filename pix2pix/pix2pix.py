...
def unet_generator(output_channels, norm_type='batchnorm'):
  ...
  # args downsample/upsample(filters, size)
  down_stack = [
    downsample(64, k_size, apply_batchnorm=False), # (bs, 128, 128, 64)
    downsample(128, k_size), # (bs, 64, 64, 128)
    downsample(256, k_size), # (bs, 32, 32, 256)
    downsample(512, k_size), # (bs, 16, 16, 512)
    downsample(512, k_size), # (bs, 8, 8, 512)
    downsample(512, k_size), # (bs, 4, 4, 512)
    downsample(512, k_size), # (bs, 2, 2, 512)
    downsample(512, k_size), # (bs, 1, 1, 512)
  ]

  up_stack = [
    upsample(512, k_size, apply_dropout=True), # (bs, 2, 2, 1024)
    upsample(512, k_size, apply_dropout=True), # (bs, 4, 4, 1024)
    upsample(512, k_size, apply_dropout=True), # (bs, 8, 8, 1024)
    upsample(512, k_size), # (bs, 16, 16, 1024)
    upsample(256, k_size), # (bs, 32, 32, 512)
    upsample(128, k_size), # (bs, 64, 64, 256)
    upsample(64, k_size), # (bs, 128, 128, 128)
  ]

  initializer = tf.random_normal_initializer(0., 0.02)
  last = tf.keras.layers.Conv2DTranspose(OUTPUT_CHANNELS, 6,
                                         strides=2,
                                         padding='same',
                                         kernel_initializer=initializer,
  ...
  return tf.keras.Model(inputs=inputs, outputs = [x,embedding])
